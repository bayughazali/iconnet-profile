  ...
  <div class="modal-dialog modal-dialog-centered modal-lg">
    <div class="modal-content rounded-4">
      ...
      <div class="modal-body">
        <p id="promoDetailDescription" class="mb-3"></p>

        <ul class="list-group list-group-flush">
          <li class="list-group-item">
            <strong>Region:</strong> <span id="promoDetailRegion"></span>
          </li>
          <li class="list-group-item">
            <strong>Diskon:</strong> <span id="promoDetailDiscount"></span>%
          </li>
          <li class="list-group-item">
            <strong>Periode:</strong> <span id="promoDetailPeriod"></span>
          </li>
        </ul>
      </div>
      ...
      <div class="modal-footer">
        ...
        <button class="btn btn-order" id="promoOrderBtn">Pesan Sekarang</button>
      </div>
...
async function showPromoDetail(promoId) {
    try {
        ...
        if (result.success) {
            ...
            document.getElementById('promoDetailDescription').innerText = promo.description;
            document.getElementById('promoDetailRegion').innerText = promo.region.toUpperCase();
            document.getElementById('promoDetailDiscount').innerText = promo.discount_percentage;
            document.getElementById('promoDetailPeriod').innerText =
                `${formatDate(promo.start_date)} - ${formatDate(promo.end_date)}`;

            // 🔹 BUTTON PESAN DI MODAL
            document.getElementById('promoOrderBtn').onclick = function () {
                orderPromo(promo.id);
            };

            // 🔹 TAMPILKAN MODAL DI TENGAH HALAMAN
...
function orderPromo(promoId) {
    const confirmation = confirm(
        'Anda akan dihubungi oleh tim kami untuk proses pemesanan promo ini. Lanjutkan?'
    );
