...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.addEventListener('click', (e) => {
    ...
    const kecepatan = card.querySelector('.package-speed')?.innerText || '';

    const hargaAfter = card.dataset.hargaAfter || '';
    const hargaBefore = card.dataset.hargaBefore || '';
    ...
    if (hargaBefore && hargaBefore > 0) {
      modalHargaBefore.textContent = formatRupiah(hargaBefore);
      modalHargaBefore.style.display = 'block';
    } else {
      modalHargaBefore.style.display = 'none';
    }

    // Harga promo
    modalHarga.textContent = formatRupiah(hargaAfter);

    // ===== Text WhatsApp (TANPA CORET) =====
    btnWhatsapp.dataset.text =
`Halo, saya ingin pesan paket ${nama}
Kecepatan: ${kecepatan}
Harga Promo: ${formatRupiah(hargaAfter)}`;
