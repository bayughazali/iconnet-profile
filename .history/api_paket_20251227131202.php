    ...
    if ($method === 'GET') {
        ...
        while ($row = $result->fetch_assoc()) {
            $paket[] = [
                ...
                // ===== HARGA BULANAN =====
                'harga_sumatera_before' => toInt($row['harga_sumatera_before']),
                'harga_sumatera'        => toInt($row['harga_sumatera']),

                'harga_jawa_before'     => toInt($row['harga_jawa_before']),
                'harga_jawa'            => toInt($row['harga_jawa']),

                'harga_timur_before'    => toInt($row['harga_timur_before']),
                'harga_timur'           => toInt($row['harga_timur']),

                // 🔽🔽 TARUH KODE KAMU DI SINI 🔽🔽
                'harga_sumatera_before' => toInt($row['harga_sumatera_before'] ?? 0),
                'harga_jawa_before'     => toInt($row['harga_jawa_before'] ?? 0),
                'harga_timur_before'    => toInt($row['harga_timur_before'] ?? 0),


                // ===== INSTALASI =====
                'instalasi_sumatera_before' => toInt($row['instalasi_sumatera_before']),
                'instalasi_sumatera'        => toInt($row['instalasi_sumatera']),

                'instalasi_jawa_before'     => toInt($row['instalasi_jawa_before']),
                'instalasi_jawa'            => toInt($row['instalasi_jawa']),

                'instalasi_timur_before'    => toInt($row['instalasi_timur_before']),
