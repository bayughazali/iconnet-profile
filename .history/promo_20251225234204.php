...
<?php foreach ($promos as $promo): ?>
    <div class="promo-card">
        ...
        <button class="btn btn-order btn-sm pesan-sekarang"
            ...
            data-nama="<?= htmlspecialchars($promo['title']) ?>"
            data-deskripsi="<?= htmlspecialchars($promo['description']) ?>"
            data-region="<?= htmlspecialchars($promo['region']) ?>"
            data-diskon="<?= number_format($promo['discount'],2) ?>%"
            data-periode="<?= $promo['start_date'].' - '.$promo['end_date'] ?>">
            Pesan Sekarang →
        </button>
    </div>
<?php endforeach; ?>
  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content rounded-4">
      ...
      <div class="modal-body">
        ...
        <p><b>Region:</b> <span id="modalRegion"></span></p>
        <p><b>Diskon:</b> <span id="modalDiskon"></span></p>
        <p><b>Periode:</b> <span id="modalPeriode"></span></p>
...
document.addEventListener('DOMContentLoaded', function () {
    document.querySelectorAll('.pesan-sekarang').forEach(button => {
        button.addEventListener('click', function () {
            ...
            document.getElementById('modalRegion').innerText = this.dataset.region;
            document.getElementById('modalDiskon').innerText = this.dataset.diskon;
            document.getElementById('modalPeriode').innerText = this.dataset.periode;
        });
    });
});
</script>
